...
export function StoreProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    try {
      localStorage.setItem(
        STORAGE_KEY,
        JSON.stringify({ version: DATA_VERSION, publishedData, draftData, orders })
      );
    } catch (e) {
      console.error(e);
    }
  }, [publishedData, draftData, orders]);
  ...
  const updateProduct = (id: number, data: Partial<Product>) =>
    setDraft(prev => ({ ...prev, products: prev.products.map(p => p.id === id ? { ...p, ...data } : p) }));

  const addProduct = (product: Omit<Product, "id">) =>
    setDraft(prev => ({
      ...prev,
      products: [...prev.products, { ...product, id: Math.max(0, ...prev.products.map(p => p.id)) + 1 }],
    }));

  const deleteProduct = (id: number) =>
    setDraft(prev => ({ ...prev, products: prev.products.filter(p => p.id !== id) }));

  const updateCollection = (index: number, data: Partial<Collection>) =>
    setDraft(prev => ({
      ...prev,
      collections: prev.collections.map((c, i) => i === index ? { ...c, ...data } : c),
    }));

  const updateTestimonial = (index: number, data: Partial<Testimonial>) =>
    setDraft(prev => ({
      ...prev,
      testimonials: prev.testimonials.map((t, i) => i === index ? { ...t, ...data } : t),
    }));

  const updateCraftStep = (index: number, data: Partial<CraftStep>) =>
    setDraft(prev => ({
      ...prev,
      craftSteps: prev.craftSteps.map((s, i) => i === index ? { ...s, ...data } : s),
    }));

  const updateStoreInfo = (data: Partial<StoreInfo>) =>
    setDraft(prev => ({ ...prev, storeInfo: { ...prev.storeInfo, ...data } }));
